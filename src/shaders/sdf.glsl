...
vec4 sdBoxGradient(in vec3 p, in vec3 b, in float r)
{
    vec3 w = abs(p) - (b - r);
...
FieldResult executeDistanceProgram(vec3 in_position, bool compute_gradients) {
    float distance_stack[16];
    vec3 gradient_stack[16];
        ...
        switch (inst.csg_op) {
            case CSG_BOX:
            {
                ...
                if (compute_gradients) {
                    gradient_stack[stack_pointer] = sdBoxGradient(point, instructions_box[inst.stream_index].bounds, 0).yzw;
                }
            ...
            case CSG_SPHERE:
            {
                ...
                if (compute_gradients) {
                    gradient_stack[stack_pointer] = sdSphereGradient(point, instructions_sphere[inst.stream_index].radius).yzw;
                }
            ...
            case CSG_BINARY_OP_UNION:
            {
                ...
                vec3 gradient_1 = gradient_stack[stack_pointer];
    ...
    result.gradient = gradient_stack[stack_pointer - 1];
